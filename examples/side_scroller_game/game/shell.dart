/// O jogo inteiro, menos a porta de entrada: a árvore de widgets, o teclado,
/// o HUD e o desenho.
///
/// Setas ou A/D correm, espaço, W ou seta para cima pulam — segure para pular
/// mais alto. ESC pausa, R recomeça.
///
/// ## Why this is not `main.dart`
///
/// It was, and it could not be compiled for a browser. The entrypoint reads
/// `arguments`, `Platform.environment`, `stdout` and `stderr`, and it names
/// `PlatformBackendResolver.defaultBackends` — four things `dart:io` owns and a
/// page has none of. A conditional import would have had to abstract all four
/// *and* the presentation list, which on the web is a WebGPU entry and a WebGL2
/// entry written out by hand and on the desktop is a resolver that picks by
/// operating system: not one seam with two implementations, but two entirely
/// different programs that happen to mount the same widget.
///
/// So the widget moved here and the two entrypoints stayed apart:
/// `main.dart` is the native one — argv, the `--script` harness, the report on
/// stdout — and `web/main.dart` is the browser one. `main.dart` re-exports this
/// file, so anything that imported the game from there still sees
/// [SideScrollerGame] and [GameSession] at the name it always had.
///
/// **Nothing in this file may import `dart:io`, and nothing may import
/// `main.dart`.** Either one puts the browser build back where it was.
///
...
///      by the tests and not by the window. **In a browser this is the ordinary
///      case and not the pathological one** until a mesh pipeline lands there:
///      `MeshSceneScope.maybeOf` answers null in a page, every frame goes
///      through [MeshRasterizer], and the game runs correct and slow rather
///      than fast and wrong.
...
///
/// The same numbers in a browser, and that is not a coincidence to be relied on
/// quietly: `dom_input_translation.dart` reports `KeyboardEvent.keyCode` as
/// [KeyEvent.logicalKey], and `keyCode` for `KeyA` is 65 on every browser
/// because it is the legacy Windows virtual-key table. The web backend's
/// library comment says so explicitly.
