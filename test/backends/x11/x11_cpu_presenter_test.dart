...
void main() {
  ...
  late X11CpuPresenter presenter;
  ...
  setUp(() {
    ...
    diagnostics.clear();
    errors.clear();
    presenter = X11CpuPresenter.withSurfaceProvider(
  ...
  test('rasterises a display list into the X11 framebuffer', () async {
  ...
  test('direct rasterisation respects native row padding', () async {
  ...
  test('an expose reuploads retained pixels and forwards its damage', () async {
  ...
  test('expose upload exceptions reach the error callback', () async {
    await presenter.renderDisplayList(redRectangle(), clearColor: 0);
...
final class _FakeSurface implements X11CpuSurface {
  _FakeSurface({
    required int width,
    required int height,
  ...
  @override
  String get kind => 'fake-x11-put-image';
