...
/// Vulkan mesh pipeline, compared pixel by pixel.
///
...
///     than sixteen levels**, which counts pixels where the two disagreed
///     about *coverage* rather than about colour.
///
/// ## What this file is really checking, which is not the same thing
///
/// The Direct3D ports transcribe HLSL that a driver compiles. This one emits
/// **SPIR-V by hand**, instruction by instruction, from the builder in
/// `vulkan_spirv.dart` - there is no glslang here and no shader compiler of any
/// kind. So a matching picture is evidence about the *emitter* as much as about
/// the pipeline: an `OpSelect` with a scalar condition where the operands are
/// vectors, a missing `NoPerspective`, a `Round` where the CPU rounds half away
/// from zero, a push-constant member at the wrong offset - each of those is a
/// module that assembles, loads, draws, and draws something subtly else.
///
/// Measured at 256x192 on an Intel(R) UHD Graphics reporting Vulkan 1.4.323,
/// beside the Direct3D 11 file's numbers on the same adapter:
///
...
/// | open double-sided shell | 38 | 34 | 1 | 1 |
/// | tiled checkerboard, unlit | **9** | 151 | 9 | 9 |
///
/// Six of the seven rows are the Direct3D 11 table's digit for digit, on a
/// different API, a different clip convention and a shader written by a
/// different program. The seventh is *better*: 9 pixels where Direct3D differs
/// on 113. That row is the one whose count was never fixed by a specification
/// - it depends on where the adapter's interpolator puts a pixel centre that
/// lands within a float of a texel boundary - and this sampler happens to round
/// the same way `MeshRasterizer`'s `(u * width).floor()` does more often. It is
/// not an improvement anything here can claim credit for, and its budget stays
/// at the Direct3D file's 0.5% for exactly that reason: the number that moved
/// is the one that is allowed to move.
///
/// The three zero rows are what say the hand-written SPIR-V is right. A cube
/// with straight silhouettes, six single-texel textures and two crossing quads
/// leave no room for a fill rule, an interpolator or a depth format to differ,
...
/// crashing. On Vulkan both live in a `VkPipeline`, so [VulkanMeshRenderer]
/// keys its pipeline cache on them: a perturbation that returned the cached
/// object would make the sabotage pass by doing nothing. Measured: clearing
/// `frontCounterClockwise` moves **33.3%** of the frame past sixteen levels,
/// reversing the depth comparison **41.9%**, and disabling the depth test
/// **17.1%** - against budgets of 0.02%. Those three numbers are the Direct3D
/// 11 file's to the third decimal, which says the three pipelines are wrong in
...
/// other. `tool/vulkan_mesh_probe.dart` is where real models are measured.
///
/// It skips rather than fails where no driver answers, which on the CI runners
/// is every run.
library;
...
void main() {
  ...
  group('the CPU and the Vulkan mesh pipeline draw the same model', () {
    ...
    test('a sphere, smooth: 73 px, none over 1 level', () async {
      ...
      // was measured and what is guaranteed: Vulkan fixes triangle coverage
    ...
    test('an open double-sided shell: 38 px, 1 over 16 levels', () async {
      ...
      //
      // Observed: 38 pixels differ, 1 by more than one level and that same 1 by
      // more than sixteen - a single point on the rim, where the bowl's
      // silhouette is a fold rather than an edge.
    ...
    test('a tiled checkerboard, wrapping: 9 px', () async {
      ...
      //
      // Observed: **9** pixels of 49 152, 0.018%, where Direct3D 11 differs on
      // 113. The budget stays at the Direct3D file's 0.5% and is not tightened
      // to match: this is the one count in the file that depends on where the
      // adapter's interpolator puts a pixel centre near a texel boundary
      // rather than on a coverage rule any specification fixes, so it is the
      // one number that is *allowed* to move between drivers. Tightening it to
      // what this driver happens to do would turn a measurement into a
      // coincidence.
      //
      // The scene's real job is the addressing mode, and it is the reason the
      // Vulkan pipeline carries a sampler and a descriptor pool of its own:
      // the device's two samplers clamp, and with `CLAMP` here this scene
      // differs on **15 646** pixels - a third of the frame, one tile smeared
      // over the twenty-four the CPU draws. See point 5 of
      // `vulkan_mesh_pipeline.dart`'s library comment.
  ...
  group('the pipeline is cached and the test can fail', () {
    test('geometry is uploaded once and drawn many times', () async {
      ...
      final VulkanMeshRenderer renderer = live.renderer;
      final Mesh3D mesh = _sphere();
      final VulkanOffscreenTarget target = live.target(_width, _height);
      try {
    ...
    test('flipping the winding leaves the model hollow', () async {
      ...
      // the clockwise triangle; Vulkan calls the clockwise triangle
      ...
      await _expectSabotage(
        session,
        _sphere(),
        apply: (VulkanMeshRenderer renderer) =>
            renderer.frontCounterClockwise = false,
        restore: (VulkanMeshRenderer renderer) =>
    ...
    test('reversing the depth comparison draws the far surface first',
      ...
      await _expectSabotage(
        ...
        _crossingQuads(),
        apply: (VulkanMeshRenderer renderer) =>
            renderer.depthTest = VulkanMeshDepthTest.greater,
        restore: (VulkanMeshRenderer renderer) =>
            renderer.depthTest = VulkanMeshDepthTest.less,
        atLeast: 0.05,
      );
    });

    test('the validation layer said nothing, or said it was absent', () {
      // The half of this file the pixels cannot reach. See
      // [_MeshSession.validationProblems]: an image that matches the CPU can
      // still have been produced by a pass whose attachment layouts, subpass
      // dependencies or descriptor-set compatibility are wrong, and this is
      // the only thing in the repository that would say so.
    ...
    test('turning the depth test off lets far triangles overwrite near ones',
      ...
      await _expectSabotage(
        ...
        _crossingQuads(),
        apply: (VulkanMeshRenderer renderer) =>
            renderer.depthTest = VulkanMeshDepthTest.always,
        restore: (VulkanMeshRenderer renderer) =>
            renderer.depthTest = VulkanMeshDepthTest.less,
        atLeast: 0.02,
      );
...
Future<void> _expectParity(
  ...
  final VulkanOffscreenTarget target = session.target(_width, _height);
  try {
    ...
    expect(
      ...
      greaterThan(0.05),
      reason:
          'the Vulkan frame is ${(difference.gpuCoverage * 100).toStringAsFixed(1)}% model, so the comparison below would pass on '
          'an empty frame',
    );
    expect(
...
Future<void> _expectSabotage(
  _MeshSession session,
  Mesh3D mesh, {
  required void Function(VulkanMeshRenderer) apply,
  required void Function(VulkanMeshRenderer) restore,
  ...
  final VulkanMeshRenderer renderer = session.renderer;
  final VulkanOffscreenTarget target = session.target(_width, _height);
...
final class _MeshSession {
  _MeshSession._(this._session, this.device, this._renderer, this.skipReason);

  final VulkanSession? _session;
  final VulkanRenderDevice? device;
  final VulkanMeshRenderer? _renderer;

  /// Null when the device opened. A sentence when it did not, so a run with no
  /// driver reports what was missing rather than passing quietly.
  ...
  static _MeshSession open() {
    ...
    if (session.skipReason != null) {
      return _MeshSession._(session, null, null, session.skipReason);
    }
    final VulkanRenderDevice device;
    try {
      ...
      return _MeshSession._(
          null, null, null, 'no Vulkan render device: $error');
    }
    final Object built = VulkanMeshRenderer.create(device);
    if (built is BackendDiagnostic) {
      device.dispose();
      session.close();
      return _MeshSession._(null, null, null,
          'the mesh pipeline was refused: ${built.message}; ${built.detail}');
    }
    return _MeshSession._(session, device, built as VulkanMeshRenderer, null);
  }
