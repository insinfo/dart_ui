...
/// Direct3D 11 mesh pipeline, compared pixel by pixel.
///
/// `d3d11_cpu_parity_test.dart` does this for the 2D display list and states
/// the argument for why a comparison is worth more than any assertion written
/// by hand: every other file checks one implementation against an expectation
/// written by whoever wrote that implementation, which catches a wrong number
/// and misses a wrong *idea*. That argument is not repeated. What is repeated
/// is the rule: **one scene, two renderers, one subtraction, and no reference
/// written by hand.**
///
/// ## Why the tolerance is not a single number here, and what it is instead
///
/// The 2D file gets to declare `tolerance: 0` on every scene, because both
/// paths rasterise the same analytic coverage of the same axis-aligned
/// rectangle. Two *triangle* rasterisers cannot be held to that everywhere, and
/// the reason is not rounding:
///
///   * **The fill rules differ, and legitimately.**
///     `MeshRasterizer._drawProjected` accepts a pixel whose three edge
///     functions all share the winding's sign, zero included, so a pixel
///     exactly on a shared edge is drawn twice - harmless under a depth test,
///     and its own comment says so. Direct3D's rule is top-left: a pixel centre
///     exactly on a right or bottom edge belongs to neither triangle. Along a
///     silhouette there can therefore be pixels one renderer paints with the
///     model and the other leaves as background, and the difference there is
///     not small - it is the whole distance from a lit surface to the clear
///     colour.
///   * **The arithmetic is float32 on one side and float64 on the other.** The
///     CPU computes barycentric weights, normals and texture coordinates in
///     Dart doubles; a pixel shader has floats. On a smooth-shaded curve that
///     moves a gradient by a fraction of a level, and on a tie - where
///     `value * intensity` lands exactly on `k + 0.5` - by one whole level.
///
/// A single "max channel deviation" would therefore have to be 255 to pass,
/// which measures nothing. So the budget is **two fractions with two different
/// meanings**:
///
///   * `shadingBudget` - the fraction of pixels whose worst channel differs by
///     **more than one level**. One level is rounding and is not counted;
///     anything above it is arithmetic that diverged.
///   * `geometryBudget` - the fraction whose worst channel differs by **more
///     than sixteen levels**. Sixteen is far above any rounding these scenes
///     produce and far below the distance from model to background, so it
///     counts pixels where the two disagreed about *coverage* rather than about
///     colour.
///
/// ### What was measured, and why most budgets are zero
///
/// The expectation going in was a few hundred silhouette pixels per scene. What
/// the device actually produced, at 256x192 on an Intel UHD Graphics at feature
/// level 11_1:
///
...
///
/// Three scenes are **identical**, and the sphere - 2 048 triangles with a
/// curved silhouette - differs nowhere by more than a single level. The fill
/// rules can differ at a silhouette and on these scenes they did not, which is
/// worth stating rather than rounding up into a margin.
///
...
/// the frame** - ten pixels, about 2% of a silhouette - wherever a silhouette
/// is curved, which is room for a tie rounding the other way on another
/// adapter and for nothing structural; and **0.5%** on the tiled texture alone,
/// which is the one scene whose count depends on the adapter's interpolator
/// rather than on Direct3D's exactly specified coverage rules. Each scene's
/// comment carries its own observation and the argument for its own number.
///
/// Direct3D's rasterisation rules being exact is what lets the tight budgets be
/// tight: coverage is fixed by the specification, so a silhouette that matches
/// here matches on every adapter, and only the shading arithmetic can move.
///
/// ### The budgets can fail
///
/// A tolerance nothing can violate is not a tolerance. The last group perturbs
/// the two pieces of state that can be wrong without crashing and records what
/// it measured: flipping `FrontCounterClockwise` moves **33.3%** of the frame
/// past sixteen levels, reversing the depth comparison **41.9%**, and disabling
/// the depth test **17.1%** - against budgets of 0.02%, or between eight
/// hundred and two thousand times the margin.
///
/// ## The scenes are generated, not loaded
///
/// No file on disk. A parity test that needed `D:/3d/sonic.glb` would pass on
/// one machine and skip on every other, and the shapes that exercise the
/// pipeline are the ones a loop can write: a closed cube for the winding, a
/// sphere for the smooth normals and a curved silhouette, an open shell for the
/// double-sided back face, a tiled quad for the sampler's `REPEAT`, and two
/// crossing quads for the depth test.
///
/// `tool/mesh_gpu_probe.dart` is where real models are measured; it prints the
/// same three numbers for any file on disk.
///
...
void main() {
  ...
  group('the CPU and the Direct3D 11 mesh pipeline draw the same model', () {
  ...
  group('the pipeline is cached and the test can fail', () {
    test('geometry is uploaded once and drawn many times', () async {
      ...
      final D3d11MeshRenderer renderer = live.renderer;
      final Mesh3D mesh = _sphere();
      final D3d11OffscreenTarget target = live.target(_width, _height);
      try {
    ...
    test('flipping the winding leaves the model hollow', () async {
      ...
      await _expectSabotage(
        session,
        _sphere(),
        apply: (D3d11MeshRenderer renderer) =>
            renderer.frontCounterClockwise = false,
        restore: (D3d11MeshRenderer renderer) =>
    ...
    test('reversing the depth comparison draws the far surface first',
      ...
      await _expectSabotage(
        ...
        _crossingQuads(),
        apply: (D3d11MeshRenderer renderer) =>
            renderer.depthTest = D3d11MeshDepthTest.greater,
        restore: (D3d11MeshRenderer renderer) =>
            renderer.depthTest = D3d11MeshDepthTest.less,
        atLeast: 0.05,
      );
    });
    ...
    test('turning the depth test off lets far triangles overwrite near ones',
      ...
      await _expectSabotage(
        ...
        _crossingQuads(),
        apply: (D3d11MeshRenderer renderer) =>
            renderer.depthTest = D3d11MeshDepthTest.always,
        restore: (D3d11MeshRenderer renderer) =>
            renderer.depthTest = D3d11MeshDepthTest.less,
        atLeast: 0.02,
      );
...
Future<void> _expectParity(
  ...
  final D3d11OffscreenTarget target = session.target(_width, _height);
  try {
...
Future<void> _expectSabotage(
  _MeshSession session,
  Mesh3D mesh, {
  required void Function(D3d11MeshRenderer) apply,
  required void Function(D3d11MeshRenderer) restore,
  ...
  final D3d11MeshRenderer renderer = session.renderer;
  final D3d11OffscreenTarget target = session.target(_width, _height);
...
final class _MeshSession {
  ...
  final D3d11MeshRenderer? _renderer;
  ...
  D3d11MeshRenderer get renderer => _renderer!;
  ...
  static _MeshSession open() {
    ...
    try {
      ...
      return _MeshSession._(null, null, 'no D3D11 device: $error');
    }
    ...
    return _MeshSession._(device, built as D3d11MeshRenderer, null);
  }
