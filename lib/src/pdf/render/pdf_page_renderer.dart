...
class PdfPageRenderer {
  ...
  void render(
    ...
    bool ignoreContentStreamErrors = false,
  }) {
    device.saveState();
    try {
      if (scale != 1.0) {
      ...
      final contents = page.readContents(
        ignoreStreamErrors: ignoreContentStreamErrors,
      );
      if (contents.bytes.isNotEmpty) {
        final interpreter = PdfContentInterpreter(
          device: device,
          resources: page.resources,
          resolver: page.resolver,
        );
        interpreter.execute(contents.bytes);
      }
    } finally {
      device.restoreState();
    }
  ...
  PdfMemoryOutputDevice renderToMemory({
    double scale = 1.0,
    bool ignoreContentStreamErrors = false,
  }) {
    ...
    render(
      dev,
      scale: scale,
      ignoreContentStreamErrors: ignoreContentStreamErrors,
    );
    return dev;
  }
}
...
extension PdfPageRenderExtension on PdfPage {
  ...
  PdfMemoryOutputDevice renderToMemory({
    ...
    return PdfPageRenderer(this).renderToMemory(
      scale: scale,
      ignoreContentStreamErrors: ignoreContentStreamErrors,
    );
  }
