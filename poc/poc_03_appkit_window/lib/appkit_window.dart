...
// Callbacks
typedef AppCallbackNative = Void Function(Pointer<ObjCObject> self, Pointer<ObjCSel> _cmd, Pointer<ObjCObject> notification);

void _onFinishLaunching(Pointer<ObjCObject> self, Pointer<ObjCSel> _cmd, Pointer<ObjCObject> notification) {
  ...
  final titleObj = msgSendPointerPointer(nsString, sel('initWithUTF8String:'), titleUtf8.cast());
  ...
  msgSendTimer(nsTimerClass, sel('scheduledTimerWithTimeInterval:target:selector:userInfo:repeats:'), 3.0, self, sel('stopApp:'), nullptr, false);
}

void _stopApp(Pointer<ObjCObject> self, Pointer<ObjCSel> _cmd, Pointer<ObjCObject> timer) {
...
void runAppKitWindow() {
  ...
  msgSendVoidIntPtr(sharedApp, sel('setActivationPolicy:'), NSApplicationActivationPolicyRegular);
  
  final delegateClassName = 'DartAppDelegate'.toNativeUtf8();
  final nsObjectClass = getClass('NSObject');
  final delegateClass = objc_allocateClassPair(nsObjectClass, delegateClassName, 0);
  ...
  finishCallback = NativeCallable<AppCallbackNative>.isolateLocal(_onFinishLaunching);
  class_addMethod(delegateClass, didFinishSel, finishCallback.nativeFunction.cast<Void>(), typesUtf8);

  final stopSel = sel('stopApp:');
  stopCallback = NativeCallable<AppCallbackNative>.isolateLocal(_stopApp);
  class_addMethod(delegateClass, stopSel, stopCallback.nativeFunction.cast<Void>(), typesUtf8);
