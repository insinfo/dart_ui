/// Renders a model through the **Direct3D 12 mesh pipeline** and prints what
...
/// numbers it prints are the ones `test/backends/win32/d3d12/`
/// `d3d12_mesh_cpu_parity_test.dart` asserts; this is where they are measured
/// and where a new model can be measured without writing a test.
///
/// ```
/// dart run tool/d3d12_mesh_probe.dart D:/3d/sonic.glb --diff
/// dart run tool/d3d12_mesh_probe.dart D:/3d/model.stl --size=1080x780 --frames=60
/// ```
///
...
///
/// Exit codes: 0 when it drew, 2 when this machine has no Direct3D 12 device or
...
Future<void> main(List<String> arguments) async {
  ...
  if (paths.isEmpty) {
    stderr.writeln('uso: dart run tool/d3d12_mesh_probe.dart <modelo> '
  ...
  if (!Platform.isWindows) {
    stderr.writeln('D3D12_MESH_GPU=SKIP platform=${Platform.operatingSystem}');
    exitCode = 2;
    return;
  }
  ...
  try {
    ...
    final D3d12RenderDevice? opened = attempt.device;
    if (opened == null) {
      stderr.writeln('D3D12_D3D12_MESH_GPU=SKIP reason=${attempt.failureText}');
      exitCode = 2;
      return;
    }
    device = opened;
  } on Object catch (error) {
    stderr.writeln('D3D12_D3D12_MESH_GPU=SKIP reason=no D3D12 device: $error');
    exitCode = 2;
    return;
  }
  ...
  if (built is BackendDiagnostic) {
    stderr.writeln(
        'D3D12_MESH_GPU=FAIL ${built.message}\n  ${built.detail ?? ''}');
    device.dispose();
    exitCode = 1;
    return;
  }
