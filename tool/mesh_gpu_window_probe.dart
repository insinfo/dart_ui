/// Draws a model with the Direct3D 11 mesh pipeline **into a real window**, and
...
/// drives mesh frames through the same `D3d11WindowTarget` and times them.
...
///
/// ```
/// dart run tool/mesh_gpu_window_probe.dart D:/3d/model.stl --frames=240
/// ```
///
/// Prints one `MESH_WINDOW=` verdict. Exit 0 when it drew and presented every
...
/// Direct3D 11 presentation, no window, no model.
library;
...
Future<void> main(List<String> arguments) async {
  if (!Platform.isWindows) {
    stderr.writeln('MESH_WINDOW=SKIP platform=${Platform.operatingSystem}');
    exitCode = 2;
    return;
  }
  ...
  if (paths.isEmpty) {
    stderr.writeln('uso: dart run tool/mesh_gpu_window_probe.dart <modelo> '
  ...
  if (!file.existsSync()) {
    stderr.writeln('MESH_WINDOW=SKIP reason=não encontrei ${paths.first}');
    exitCode = 2;
    return;
  }
  ...
  try {
    ...
    stderr.writeln('MESH_WINDOW=SKIP reason=${error.message}');
    exitCode = 2;
    return;
  }
  ...
  try {
    app = await Application.start(
      ...
      options: ApplicationOptions(
        title: 'dart_ui Direct3D 11 mesh probe',
        ...
        // GPU drawing them.
        visible: true,
        // gpuOnly, so a machine whose Direct3D 11 probe fails is a loud failure
        // instead of a CPU frame that would prove nothing.
        renderingPolicy: RenderingPolicy.gpuOnly,
        requestedPresentation: 'direct3d11',
        onError: errors.add,
        onDiagnostic: diagnostics.add,
      ),
    );
  } on Object catch (error) {
    stderr.writeln('MESH_WINDOW=SKIP reason=a aplicação não abriu: $error');
    exitCode = 2;
    return;
  }
  ...
  try {
    ...
    if (chosen != 'direct3d11') {
      stderr.writeln('MESH_WINDOW=SKIP reason=a seleção escolheu $chosen');
      exitCode = 2;
      return;
    }
    final SurfacePresenter presenter = app.host.presenter;
    if (presenter is! RenderTargetPresenter) {
      stderr.writeln('MESH_WINDOW=SKIP reason=${presenter.runtimeType} não '
          'tem RenderTarget');
      exitCode = 2;
      return;
    }
    final RenderTarget raw = presenter.target;
    if (raw is! D3d11WindowTarget) {
      stderr.writeln('MESH_WINDOW=SKIP reason=${raw.runtimeType} não é '
          'D3d11WindowTarget');
      exitCode = 2;
      return;
    }
    final D3d11RenderDevice device = _deviceOf(raw);
    final Object built = D3d11MeshRenderer.create(device);
    if (built is BackendDiagnostic) {
      stderr.writeln('MESH_WINDOW=FAIL ${built.message}: ${built.detail}');
      exitCode = 1;
      return;
    }
    ...
    try {
      ...
      if (renderer.bufferUploadCount > mesh.primitives.length * 2) {
        stderr.writeln('MESH_WINDOW=FAIL ${renderer.bufferUploadCount} buffer '
      ...
      if (_p95(frameUs) > median * 3 ~/ 2) {
        stderr.writeln('MESH_WINDOW=FAIL p95 ${_ms(_p95(frameUs))} ms against '
            'a median of ${_ms(median)} ms: frames are missing the refresh');
        ok = false;
      }
      if (notPresented > 0) {
        stderr.writeln('MESH_WINDOW=FAIL $notPresented of ${frameUs.length} '
      ...
      if (!resizedOk) {
        stderr.writeln('MESH_WINDOW=FAIL the mesh pass stopped presenting '
            'after a resize');
        ok = false;
      }
      ...
      if (after.status != PresentStatus.presented) {
        stderr.writeln('MESH_WINDOW=FAIL the 2D pass after the mesh pass did '
            'not present: $after');
        ok = false;
      }
    } finally {
    ...
    if (errors.isNotEmpty) {
      stderr.writeln('MESH_WINDOW=FAIL ${errors.length} framework errors');
      ok = false;
    }
  } finally {
  ...
  stdout.writeln('MESH_WINDOW=${ok ? 'PASS' : 'FAIL'}');
  exitCode = ok ? 0 : 1;
}
...
/// `ID3D11Device` cannot be bound alongside a render-target view belonging to
/// this one, and Direct3D answers that by drawing nothing rather than by
/// failing.
D3d11RenderDevice _deviceOf(D3d11WindowTarget target) => target.device;
