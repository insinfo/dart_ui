/// Draws a model with the Vulkan mesh pipeline **into a real window**, and
...
/// drives mesh frames through the same `VulkanWindowTarget` and times them.
...
///
/// ```
/// dart run tool/vulkan_mesh_window_probe.dart D:/3d/model.stl --frames=240
/// ```
///
/// Prints one `VULKAN_MESH_WINDOW=` verdict. Exit 0 when it drew and presented every
...
/// Vulkan presentation, no window, no model.
library;
...
Future<void> main(List<String> arguments) async {
  if (!Platform.isWindows) {
    stderr.writeln(
        'VULKAN_MESH_WINDOW=SKIP platform=${Platform.operatingSystem}');
    exitCode = 2;
    return;
  }
  ...
  if (paths.isEmpty) {
    stderr.writeln('uso: dart run tool/vulkan_mesh_window_probe.dart <modelo> '
  ...
  if (!file.existsSync()) {
    stderr
        .writeln('VULKAN_MESH_WINDOW=SKIP reason=não encontrei ${paths.first}');
    exitCode = 2;
    return;
  }
  ...
  try {
    ...
    stderr.writeln('VULKAN_MESH_WINDOW=SKIP reason=${error.message}');
    exitCode = 2;
    return;
  }
  ...
  try {
    app = await Application.start(
      ...
      options: ApplicationOptions(
        title: 'dart_ui Vulkan mesh probe',
        ...
        // GPU drawing them.
        visible: true,
        // gpuOnly, so a machine whose Vulkan probe fails is a loud failure
        // instead of a CPU frame that would prove nothing.
        renderingPolicy: RenderingPolicy.gpuOnly,
        requestedPresentation: 'vulkan',
        // Vulkan is registered `experimental: true` and is never reached by
        // fallback, so naming it is not enough - the selection refuses an
        // experimental path outright without this. See
    ...
    stderr.writeln(
        'VULKAN_MESH_WINDOW=SKIP reason=a aplicação não abriu: $error');
    exitCode = 2;
    return;
  }
  ...
  try {
    ...
    if (chosen != 'vulkan') {
      stderr
          .writeln('VULKAN_MESH_WINDOW=SKIP reason=a seleção escolheu $chosen');
      exitCode = 2;
      return;
    }
    ...
    if (presenter is! RenderTargetPresenter) {
      stderr.writeln(
          'VULKAN_MESH_WINDOW=SKIP reason=${presenter.runtimeType} não '
          'tem RenderTarget');
      exitCode = 2;
      return;
    }
    final RenderTarget raw = presenter.target;
    if (raw is! VulkanWindowTarget) {
      stderr.writeln('VULKAN_MESH_WINDOW=SKIP reason=${raw.runtimeType} não é '
          'VulkanWindowTarget');
      exitCode = 2;
      return;
    }
    final VulkanRenderDevice device = _deviceOf(raw);
    final Object built = VulkanMeshRenderer.create(device);
    if (built is BackendDiagnostic) {
      stderr
          .writeln('VULKAN_MESH_WINDOW=FAIL ${built.message}: ${built.detail}');
      exitCode = 1;
      return;
    }
    ...
    try {
      ...
      if (renderer.bufferUploadCount > mesh.primitives.length * 2) {
        stderr.writeln(
            'VULKAN_MESH_WINDOW=FAIL ${renderer.bufferUploadCount} buffer '
      ...
      if (_p95(frameUs) > median * 3 ~/ 2) {
        stderr.writeln(
            'VULKAN_MESH_WINDOW=FAIL p95 ${_ms(_p95(frameUs))} ms against '
            'a median of ${_ms(median)} ms: frames are missing the refresh');
        ok = false;
      }
      if (notPresented > 0) {
        stderr.writeln(
            'VULKAN_MESH_WINDOW=FAIL $notPresented of ${frameUs.length} '
      ...
      if (!resizedOk) {
        stderr
            .writeln('VULKAN_MESH_WINDOW=FAIL the mesh pass stopped presenting '
                'after a resize');
        ok = false;
      }
      ...
      if (after.status != PresentStatus.presented) {
        stderr.writeln(
            'VULKAN_MESH_WINDOW=FAIL the 2D pass after the mesh pass did '
            'not present: $after');
        ok = false;
      }
    } finally {
    ...
    if (errors.isNotEmpty) {
      stderr
          .writeln('VULKAN_MESH_WINDOW=FAIL ${errors.length} framework errors');
      ok = false;
    }
  } finally {
  ...
  stdout.writeln('VULKAN_MESH_WINDOW=${ok ? 'PASS' : 'FAIL'}');
  exitCode = ok ? 0 : 1;
}
...
/// `VkDevice` cannot be bound alongside a render-target view belonging to
/// this one, and Direct3D answers that by drawing nothing rather than by
/// failing.
VulkanRenderDevice _deviceOf(VulkanWindowTarget target) => target.device;
